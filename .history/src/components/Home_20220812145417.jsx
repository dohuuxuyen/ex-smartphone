            ...
            {data
              ...
              .map((d, index) => (
                <div className="m-auto text-center">
                  <img
                    key={index}
                    className="w-[224px] h-[281px] border border-gray-400 rounded bg-white object-cover"
                    src={d.newsProducts}
                    alt=""
                  />
                  <h3 className="mt-4">{d.title}</h3>
                  <p className="my-3">{d.sizes[0].price}</p>
                  <button className=" rounded border-black bg-black text-white font-semibold p-2 hover:bg-white hover:text-black">
                    CHỌN MUA
                  </button>
                </div>
              ))}
