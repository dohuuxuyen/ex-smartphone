...
const Home = () => {
  ...
  return (
    <>
      <div className="w-[100%] max-w-[1200px] mx-auto mt-[70px] sm:mx-2">
        <div className="bg-black text-white flex items-center justify-center h-8 sm:mx-2 sm:h-6 sm:text-[14px]">
          Uư đãi giảm 10% khi thanh toán trả trước
        </div>
        <div className=" flex items-center justify-between  mx-auto overflow-hidden sm:mx-2">
          <div className="h-[calc(100vh-102px)] w-[400vw] flex relative z-[-1] ease-linear">
            {dataHome.map((d, index) => (
              <div
                key={index}
                style={{ transform: `translateX(-${currentSlide * 100}vw)` }}
                className="h-[90%] w-[100vw] sm:h-[80%]"
              >
                <img
          ...
          <div
            onClick={handleLeft}
            className="absolute left-5 top-[50%] text-4xl cursor-pointer opacity-50 sm:top-[45%]"
          >
            <FaAngleLeft />
          </div>
          <div
            onClick={handleRight}
            className="absolute right-5 top-[50%] text-4xl cursor-pointer opacity-50 sm:top-[45%]"
          >
            <FaAngleRight />
          </div>
        </div>
