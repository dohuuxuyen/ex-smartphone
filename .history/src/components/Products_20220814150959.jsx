...
const Products = () => {
  ...
  const FilterProducts = (e, index) => {
    const updateListFilter = data.filter((x) => x.category === e);
    setFilter(updateListFilter);
    const textActive = myRef.current.children;
    const textLength = textActive.length;
    for (let i = 0; i < textLength; i++) {
      textActive[i].className = textActive[i].className.replace(
        "text-red-500",
        ""
      );
      textActive[index].className = "text-red-500";
    }
  };
  return (
