          ...
          {state.map((cartItem, index) => (
            <div key={index} className="h-[150px] flex items-center">
              <img
                ...
                src={cartItem.product.src[tabIndex]}
                alt=""
              />

              <div className="w-[340px] font-medium">
                {cartItem.product.title} -{cartItem.product.colors[colors]} -
                {cartItem.product.sizes[size].size}
              </div>

              <div className="mx-5 w-[120px] text-center font-medium">
                {cartItem.product.sizes[size].price}
              </div>
