          ...
          {state.map((cartItem, index) => (
            <div key={index} className="h-[150px] flex items-center">
              <img
                ...
                src={cartItem.product.product.src[cartItem.tabIndex]}
                alt=""
              />

              <div className="w-[340px] font-medium">
                {cartItem.product.product.title} -
                {cartItem.product.product.colors[cartItem.colors]} -
                {cartItem.product.product.sizes[cartItem.size].size}
              </div>

              <div className="mx-5 w-[120px] text-center font-medium">
                {cartItem.product.product.sizes[cartItem.size].price}
              </div>
