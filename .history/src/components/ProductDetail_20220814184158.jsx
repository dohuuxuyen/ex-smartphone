...
const ProductDetail = () => {
  ...
  const handleTabImg = (index) => {
    setTabIndex(index);
    const abc = colorRef.current.children;
    for (let i = 0; i < abc.length; i++) {
      abc[i].className = abc[i].className.replace("active-img", "");
    }
    abc[index].className = "active-img";
  };
  return (
            ...
            <div ref={colorRef} className="flex">
              {product.colors.map((color, index) => (
                <div
                  onClick={() => handleTabImg(index)}
                  key={index}
                  className="w-8 h-8 flex border border-red-500 rounded-[50%] mr-3 mt-2 cursor-pointer"
