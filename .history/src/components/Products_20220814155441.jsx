...
const Products = () => {
  ...
  const FilterProducts = (index) => {
    const updateListFilter = data.filter(
      (x) => x.category === "xiaomi" || x.category === "iphone"
    );
    setFilter(updateListFilter);
    const abc = myRef.current.children;
    for (let i = 1; i < abc.length; i++) {
      abc[i].className = abc[i].className.replace("active", "");
    }
    abc[index].className = "active";
  };
