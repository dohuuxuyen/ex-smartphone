...
const ProductDetail = () => {
  ...
  const handleTabImg = (index) => {
    setTabIndex(index);
    const tabImgActive = imgRef.current.children;
    const tabColorActive = colorRef.current.children;
    for (let i = 0; i < tabImgActive.length; i++) {
      tabImgActive[i].className = tabImgActive[i].className.replace(
        "active-img",
        ""
      );
    }
    for (let j = 0; j < tabColorActive.length; j++) {
      tabColorActive[j].className = tabColorActive[j].className.replace(
        "active-img",
        ""
      );
    }
    tabImgActive[index].className = "active-img";
    tabColorActive[index].className = "active-img";
  };
  ...
  return (
    <div className="w-[100%] max-w-[1200px] mx-auto mt-[70px]">
      ...
      <div className="flex">
        <div ref={imgRef} className="w-[100px] h-[137px]">
        ...
        <div className="ml-20 mt-5">
          ...
          <div className=" my-5">
            ...
            <div className="flex ">
              {product.sizes.map((sp, index) => (
                <div
                  key={index}
                  className="border border-[#adadad] cursor-pointer mr-4 p-2 rounded-lg "
                >
                  <p>{sp.size}</p>
                  <p>{sp.price}</p>
                </div>
          ...
          <div>
            ...
            <div ref={colorRef} className="flex">
